...
@assets.route("/add_integration", methods=['GET', 'POST'])
@login_required
def add_integration():
    try:
        ...
        if request.method == 'POST':
            ...
            return redirect(url_for('assets.all_integrations', user=user, NAV=NAV))
        assets_all = Integrations.query.all()
        schema = IntegrationsSchema(many=True)
        assets = schema.dump(
            filter(lambda t: t.ToolType == 'Jenkins', assets_all)
        )
        return render_template('assets/add_integration.html', entities=assets, user=user, NAV=NAV)
    except RuntimeError:
        return render_template('500.html'), 500


@assets.route("/all_integrations")
@login_required
def all_integrations():
    try:
        ...
        return render_template('assets/all_integrations.html', entities=assets, user=user, NAV=NAV)
    except RuntimeError:
        return render_template('500.html'), 500


@assets.route("/validate_integration", methods=['POST'])
@login_required
...
@assets.route("/add_app_integration/<app_id>")
@login_required
def add_app_integration(app_id):
    try:
        ...
        NAV['appbar'] = 'settings'
        return render_template('assets/add_app_integration.html', entities=entities, user=user,
                               NAV=NAV, app_id=app_id, app_data=app_data)
    except RuntimeError:
        return render_template('500.html'), 500


@assets.route("/submit_app_integration/<app_id>", methods=['POST'])
@login_required
def submit_app_integration(app_id):
    try:
        ...
        return redirect(url_for('assets.all_app_integrations', app_id=app_id))
    except RuntimeError:
        return render_template('500.html'), 500


@assets.route("/all_app_integrations/<app_id>")
@login_required
def all_app_integrations(app_id):
    try:
        ...
        app_data = {'ID': app_id, 'ApplicationName': app.ApplicationName, 'Component': app.ApplicationAcronym}
        NAV['appbar'] = 'settings'
        return render_template('assets/all_app_integrations.html', entities=assets, user=user,
                               NAV=NAV, app_data=app_data)
    except RuntimeError:
        return render_template('500.html'), 500

@assets.route("/remove_app_integration", methods=['POST'])
@login_required
