...
from vr.assessments.model.assessmentbenchmarks import AssessmentBenchmarks
...
@assets.route("/all_applications", methods=['GET', 'POST'])
@login_required
def all_applications():
    ...
    try:
        ...
        return render_template('assets/all_applications.html', entity_details=entity_details, entities=assets_all.items, user=user,
                               NAV=NAV, table_details= table_details, app_data={"ID": 0})
    except RuntimeError:
        return render_template(SERVER_ERR_STATUS), 500


@assets.route("/all_applications_filtered/<type>/<val>", methods=['GET', 'POST'])
@login_required
...
@assets.route("/application/<component_id>/<app_type>/<entity_name>")
@login_required
...
@assets.route("/application/<id>/export")
@login_required
def application_export(id):
    ...
    html = render_template('assets/application_pdf.html', regs_all=data_map['regs_all'], vuln_data=data_map['vuln_data'], app_data=data_map['app_data'],
                               sla_data=data_map['sla_data'], ld=data_map['ld'], color_wheel=data_map['color_wheel'],
                               user=data_map['user'], NAV=data_map['NAV'], contacts=data_map['contacts'], dependency_map=data_map['dependency_map'],
...
@assets.route("/application/<id>/csv")
@login_required
...
@assets.route("/add_application", methods=['GET', 'POST'])
@login_required
@requires_role('Admin')
def add_application():
    try:
        ...
        if request.method == 'POST':
            ...
            return render_template('assets/add_application_integrations.html', user=user, NAV=NAV, new_app_name=app_name,
        ...
        return render_template('assets/add_application.html', user=user, NAV=NAV, product_types=product_types, all_slas=all_slas,
                               all_regs=all_regs)
    except RuntimeError:
        return render_template(SERVER_ERR_STATUS), 500
...
@assets.route("/application_issues/<id>", methods=['GET', 'POST'])
@login_required
def application_issues(id):
    try:
        ...
        return render_template('assets/application_issues.html', entities=assets, app_data=app_data, user=user, NAV=NAV,
                               table_details=table_details)
    except RuntimeError:
        return render_template(SERVER_ERR_STATUS), 500


@assets.route("/delete_application/<id>")
@login_required
